# B3-Story 2.4: Implement Data Compression
...
- [ ] Ensure opening the "Notebooks" dropdown lists all available notebooks
- [ ] Show loading indicator while notebooks are fetched/resolved
- [ ] Display empty-state with create action when no notebooks exist
- [ ] Ensure behavior works offline with cached data where available
- [ ] Add/verify tests in `src/components/layout/NotebooksSidebar.test.tsx`
- [ ] Add/verify integration test in `src/components/layout/ThreeColumnLayout.integration.test.tsx`
